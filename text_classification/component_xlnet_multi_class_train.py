...
is_training = False
tf_float = tf.bfloat16 if cf.use_bfloat16 else tf.float32
input_ids = tf.placeholder(tf.int32, shape=[None, cf.max_seq_length], name='input_ids')
input_mask = tf.placeholder(tf.float32, shape=[None, cf.max_seq_length], name='input_mask')
segment_ids = tf.placeholder(tf.int32, shape=[None, cf.max_seq_length], name='segment_ids')
labels = tf.placeholder(tf.int32, shape=[None, ], name='label_ids')
# keep_prob = tf.placeholder(tf.float32, name='keep_prob')  # , name='is_training'
(total_loss, per_example_loss, logits) = create_model(cf, input_ids, input_mask, segment_ids, labels,
                                                      is_training=False)
